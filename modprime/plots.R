...
ggplot(data1.1, aes(x = bits, y = nanos, color = scheme)) +
  geom_point(size = 2, aes(shape = is128)) +
...
ggplot(data1.2, aes(x = bits, y = nanos, color = scheme)) +
  geom_point(size = 2, aes(shape = is128)) +
  scale_x_continuous(name = "bit-length of input", breaks = 32*(0:4), limits = c(0, 128)) +
...
ggplot(data1.3, aes(x = bits, y = nanos, color = scheme)) +
  geom_point(size = 2, aes(shape = is128)) +
